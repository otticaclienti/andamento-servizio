...
export default function SpesePage() {
    ...
    if (!authLoading) {
      ...
      const timer = setTimeout(() => {
        const initialRows = mockClients
          .filter((c) => c.isActive)
          .map((c) => {
            const existingSpend = mockWeeklySpends.find(
              (s) => s.clientId === c.id && s.weekStart === weekStart
            );
            return {
              clientId: c.id,
  ...
  const handleAmountChange = (clientId: string, value: string) => {
    setRows((prev) =>
      prev.map((r) =>
        r.clientId === clientId ? { ...r, amount: value, saved: false } : r
      )
    );
  };
  ...
  const handleSave = (clientId: string) => {
    setRows((prev) =>
      prev.map((r) =>
        r.clientId === clientId ? { ...r, saved: true } : r
      )
    );
  };
  ...
  const handleSaveAll = () => {
    setRows((prev) =>
      prev.map((r) => (r.amount ? { ...r, saved: true } : r))
    );
  };
      ...
      <main className="lg:ml-64 pb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Settimana del {new Date(weekStart).toLocaleDateString('it-IT', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            </div>
            <button
        ...
        <div className="px-4 lg:px-8 py-6 max-w-3xl">
          ...
          <p className="text-xs text-gray-400 mt-4 text-center">
            Inserisci la spesa pubblicitaria di ogni cliente per questa settimana.
            I dati verranno mostrati nella dashboard del cliente.
          </p>
        </div>
      </main>
    </div>
